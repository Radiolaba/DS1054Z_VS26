...
f_max = 120000;  % Конечная частота, Гц
m = 10;         % Количество циклов для усреднения

% Читаем CSV: 3 столбца (V1, V2, t), разделитель — ';'
M  = dlmread('data_net_vint_square_25000_20us.csv',';');
...
% Вычисляем БПФ для обоих сигналов
X1 = fft(V1) / N;
X2 = fft(V2) / N;
...
% Удваиваем комплексные амплитуды
X1_single(2:end-1) = 2 * X1_single(2:end-1);
X2_single(2:end-1) = 2 * X2_single(2:end-1);

% Усреднение спектров (имитация m циклов - используем одно измерение)
Sxx = abs(X1_single).^2;           % Авто-спектр воздействия
Sxy = conj(X1_single) .* X2_single; % Кросс-спектр

% Вычисляем частотную функцию по формуле (3) из статьи
H_single = Sxy ./ Sxx;
...
H_range = H_single(idx);

% Разложение для сигнала 1
X1_real = real(X1_range);
X1_imag = imag(X1_range);
X1_mag = abs(X1_range);
X1_phase = angle(X1_range) * 180/pi;  % БЕЗ unwrap

% Разложение для сигнала 2
X2_real = real(X2_range);
X2_imag = imag(X2_range);
X2_mag = abs(X2_range);
X2_phase = angle(X2_range) * 180/pi;  % БЕЗ unwrap

% Разложение для частотной функции
H_real = real(H_range);
H_imag = imag(H_range);
H_mag = abs(H_range);
H_phase = angle(H_range) * 180/pi;    % БЕЗ unwrap
...
subplot(2,1,2);
plot(f_range, X1_phase, 'g-', 'LineWidth', 1.5);
grid on;
xlabel('f, Гц');
...
subplot(2,1,2);
plot(f_range, X2_phase, 'g-', 'LineWidth', 1.5);
grid on;
xlabel('f, Гц');
...
subplot(2,1,2);
plot(f_range, H_phase, 'g-', 'LineWidth', 1.5);
grid on;
xlabel('f, Гц');
ylabel('arg[H(f)], град');
